"""
main-cpu.py — Text Summarisation & Question Answering (CPU, float32)

Uses the model defined in CPU_LLM inside .env.
Automatically reads the single document from /text.

Compatible with any HuggingFace instruct/chat model that supports
tokenizer.apply_chat_template() — just update CPU_LLM in .env.

Note: Running a large LLM on CPU is significantly slower than GPU.
      Recommend using a smaller model (e.g. 1.5B or 3B parameters) in CPU_LLM.

Requirements:
    pip install transformers torch python-dotenv pypdf
"""
...
def load_model(model_name: str):
    """
    Load tokenizer and model from HuggingFace Hub in float32 for CPU inference.
    ...
    """
    print(f"[cpu] Loading model: {model_name}")
    print("[cpu] Running on CPU (float32) — this may be slow for large models.")
...
def main():
    print("=" * 60)
    print("  Text Summarisation & QA — CPU Mode (float32)")
    print("=" * 60)

    # 1. Load model name and generation config from .env
    model_name = get_model_name("cpu")
    gen_config = get_generation_config()

    print(f"[cpu] Temperature: {gen_config['temperature']}")
    print(f"[cpu] Max tokens (summary): {gen_config['max_new_tokens_summary']}")
    print(f"[cpu] Max tokens (QA):      {gen_config['max_new_tokens_qa']}\n")
